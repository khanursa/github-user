...
class GithubUserFollower : Fragment(), ProgressInterface {
    ...
    override fun onFailled() {
        view?.loading?.visibility = View.GONE
        Toast.makeText(requireContext(), "Gak bisa ambil data follower", Toast.LENGTH_SHORT).show()
    }

    fun bridge(username: String): GithubUserFollower {
        val dataBundle = Bundle()
        dataBundle.putString("EXTRA_STRING", username)
        val argument = GithubUserFollower()
